    @php
        $entriesData = Statamic::tag('collection')
            ->collection('products')
            ->sort('title:asc')
            ->paginate(12)
            ->as('results')
    ...
    <x-layouts.app>
        ...
        <div class="mx-auto w-full max-w-3xl px-4 py-8">
                ...
                @foreach ($entriesData['results'] ?? [] as $entry)
                    <article class="rounded-xl bg-white p-6 shadow dark:bg-zinc-950">
                        <h2 class="text-xl font-semibold">
                            <a href="{{ $entry->url }}"
                                class="text-indigo-700 hover:underline dark:text-indigo-400">{{ $entry->title }}</a>
                        </h2>
                        @if ($entry->sku)
                        ...
                        @if ($entry->description)
                            <p class="mt-3 text-zinc-600 dark:text-zinc-400">
                                {{ Str::limit(strip_tags($entry->description), 250) }}</p>
                        @endif
                    </article>
                @endforeach
            ...
            <p class="mt-8">
                <a href="/products" class="text-indigo-700 hover:underline dark:text-indigo-400">&larr; Back to
                    products</a>
            </p>
        </div>
