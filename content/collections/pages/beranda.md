---
id: home
blueprint: page
title: Beranda
template: home
updated_by: 28d34247-1c17-42bf-8548-5b36f18adcbd
updated_at: 1781343815
sections:
  -
    id: ZLGXZE_k6w_GoDXYnWc7H
    identifier: section-about
    image: truck-about.png
    heading: 'Distributor Resmi dengan Jaringan Layanan Nasional'
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'PT Gaya Makmur Mobil (GM Mobil) merupakan Agen Pemegang Merek (APM) dari First Automobile Works (FAW) yang mulai memasarkan unit FAW Truck sejak 2005. GM Mobil adalah salah satu Perusahaan dari Gaya Makmur Group yang berfokus pada penjualan truk seperti pada sektor logistik, konstruksi, pertambangan, kehutanan, dan perkebunan.'
    button:
      label: 'Tentang GM Mobil'
      link: 'entry::df438d73-5602-40d0-806d-cb9c62bbe581'
      style: primary
    images:
      - cer-1.jpg
      - cer-2.jpg
    columns: '3'
    counter_grid:
      -
        id: dJV06CuM9iMAIcko5S_hI
        number: 20
        prefix: +
        suffix: +
        caption: 'Tahun Pengalaman'
      -
        id: NH8X2YMjK7LJjdmGCbWPn
        number: 15000
        suffix: +
        caption: 'Unit Beroperasi'
      -
        id: _DiiHIxuSjew2mHf4HDmL
        number: 57
        caption: 'Service Center'
      -
        id: WR3mPXC6Io4WQzMcorxjb
        number: 24
        suffix: /7
        caption: 'Dukungan After Sales'
    show: true
    type: text_counter_image
    enabled: true
    section_images: bg-about-section.jpg
  -
    id: vy_ndF3JTqUvIww-0ySee
    identifier: section-product-category
    heading: 'Rangkaian Produk untuk Berbagai Kebutuhan Operasional'
    description: Desc
    label: 'Semua Produk'
    link: 'entry::products'
    style: primary
    type: text_button
    enabled: true
  -
    id: oQb1Ha3oFSzen0KV_lF29
    identifier: section-services
    heading: 'Layanan Kami'
    subheading: 'Dari konsultasi sebelum pembelian hingga maintenance rutin, dari emergency repair hingga program REMAN, kami hadir sebagai partner jangka panjang Anda.'
    columns: '3'
    features:
      -
        id: Q5lN_DB3sAJvr6N3wtk99
        icon: footer-background.jpg
        title: 'Layanan Purna Jual'
        text: 'Parts berkualitas OEM dengan harga lebih terjangkau. Perpanjang lifecycle armada hingga 2x lipat dengan biaya maintenance yang terkontrol.'
        link: 'entry::24ae8af8-2863-4b05-9f50-033227224acb'
      -
        id: IUYwuoOFwQtUCYvnGsSwO
        icon: bg-about-section.jpg
        title: 'Reman Center'
        text: 'Parts berkualitas OEM dengan harga lebih terjangkau. Perpanjang lifecycle armada hingga 2x lipat dengan biaya maintenance yang terkontrol.'
        link: 'entry::72127caa-9498-41f7-8735-316ab3a72ebb'
      -
        id: tIyDMbEc0ihymnnyS79eK
        icon: cta-background.jpg
        title: 'GM Teletech'
        text: 'Parts berkualitas OEM dengan harga lebih terjangkau. Perpanjang lifecycle armada hingga 2x lipat dengan biaya maintenance yang terkontrol.'
        link: 'entry::44019b3e-9005-4c58-a58d-7da1641b96d3'
    show: true
    type: feature_grid
    enabled: true
---
